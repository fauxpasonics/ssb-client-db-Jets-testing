...
CREATE PROCEDURE [etl].[TM_Load_ods_AttendApi_Archive]
(
...
DECLARE @SrcRowCount int = ISNULL((SELECT CONVERT(varchar, COUNT(*)) FROM tmarc.STG__TM_AttendApi),'0');	
...
,  [event_id], [event_name], [event_date], [section_id], [section_name], [row_id], [row_name], [seat_num], [acct_id], [plan_event_Id], [plan_event_name], [print_count], [ticket_type], [scan_type], [result_code], [action_time], [gate], [price_code], [comp], [result_type], [valid], [ticket_acct_id], [source_system], [access_type], [channel_ind], [mobile], [seq_num]
FROM tmarc.STG__TM_AttendApi



DECLARE @RecordCount INT = (SELECT COUNT(*) FROM tmarc.STG__TM_AttendApi)
...
BEGIN 
	DELETE a
	FROM tmarc.STG__TM_AttendApi a
	INNER JOIN (
	SELECT ETL__ID
...
UPDATE tmarc.STG__TM_AttendApi SET acct_id = NULL WHERE acct_id = '';

UPDATE tmarc.STG__TM_AttendApi SET event_date = NULL WHERE event_date = '';

UPDATE tmarc.STG__TM_AttendApi SET event_id = NULL WHERE event_id = '';

UPDATE tmarc.STG__TM_AttendApi SET mobile = NULL WHERE mobile = '';

UPDATE tmarc.STG__TM_AttendApi SET plan_event_Id = NULL WHERE plan_event_Id = '';

UPDATE tmarc.STG__TM_AttendApi SET print_count = NULL WHERE print_count = '';

UPDATE tmarc.STG__TM_AttendApi SET result_code = NULL WHERE result_code = '';

UPDATE tmarc.STG__TM_AttendApi SET row_id = NULL WHERE row_id = '';

UPDATE tmarc.STG__TM_AttendApi SET seat_num = NULL WHERE seat_num = '';

UPDATE tmarc.STG__TM_AttendApi SET section_id = NULL WHERE section_id = '';

UPDATE tmarc.STG__TM_AttendApi SET ticket_acct_id = NULL WHERE ticket_acct_id = '';

UPDATE tmarc.STG__TM_AttendApi SET upd_datetime = NULL WHERE upd_datetime = '';


DECLARE @MaxId INT = (SELECT MAX(ETL__ID) FROM tmarc.STG__TM_AttendApi)
