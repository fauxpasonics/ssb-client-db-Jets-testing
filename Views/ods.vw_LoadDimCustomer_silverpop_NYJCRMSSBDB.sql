...
CREATE VIEW [ods].[vw_LoadDimCustomer_silverpop_NYJCRMSSBDB] AS ( --Change "SOURCESYSTEM" to the correct source
...
--base set
		SELECT
			...
			/*AddressPrimary*/
            , CASE WHEN ISNULL(z.Address_Street_2,'') != '' THEN 
					z.Address_Street + ' ' + z.Address_Street_2 
					ELSE
					z.Address_Street END AS [AddressPrimaryStreet] -- dont load suites combine addresses with multiple lines and load to street eg. CASE WHEN ISNULL(address2,'') <> '' THEN z.street + ' ' + z.address2 ELSE z.street END AS [AddressPrimaryStreet]
			...
			, z.[Last_Send] [SSUpdatedDate] --lastmodifiedbyid
		...
		--SELECT COUNT(*)			
		FROM stg.IMC_NYJSSBDB z WITH (NOLOCK) --Change To Source Table
		WHERE 1=1
