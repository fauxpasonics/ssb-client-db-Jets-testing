...
CREATE VIEW [ods].[vw_LoadDimCustomer_silverpop_NYJCRM2011] AS ( --Change "SOURCESYSTEM" to the correct source
...
--base set
		SELECT
			...
			, 'silverpop_NYCRM2011' AS [SourceSystem] --Put source name such as TM or CRM_Contact
			...
			, CONVERT(VARCHAR(30),Date_of_Birth, 102) AS [Birthday] -- field is sensitive see writup in sharepoint for an example of how complicated it can get
			, NULL AS [Gender] 
			, 0 [MergedRecordFlag]
			, NULL [MergedIntoSSID]
			...
			/*AddressPrimary*/
            , CASE WHEN ISNULL(z.STH_Street_2,'') != '' AND ISNULL(z.STH_Street_2,'') NOT LIKE '%,%' THEN 
					z.STH_Street_1 + ' ' + z.STH_Street_2 
					ELSE
					z.STH_Street_1 END AS [AddressPrimaryStreet] -- dont load suites combine addresses with multiple lines and load to street eg. CASE WHEN ISNULL(address2,'') <> '' THEN z.street + ' ' + z.address2 ELSE z.street END AS [AddressPrimaryStreet]
			, NULL AS [AddressPrimarySuite] -- ALWAYS LEAVE THIS NULL!!!!!!!!!!!!!!!!
			, z.STH_City AS [AddressPrimaryCity] 
			, z.STH_State AS [AddressPrimaryState] 
			, z.STH_Zip AS [AddressPrimaryZip] 
			...
			/*Phone*/
			, LEFT(z.STH_Phone_Number,25) AS [PhonePrimary] -- For Coalescing values make sute they are wrapped in a NULLIF for values that = '' eg COALESCE(NULLIF(z.mobile,''),z.phone) AS [PhonePrimary]
			, LEFT(z.STH_Phone_Number,25) AS [PhoneHome]
			...
			, z.[Silverpop_LastSent] [SSUpdatedDate] --lastmodifiedbyid
		...
		--SELECT COUNT(*)			
		FROM stg.IMC_NotoriousBIG z WITH (NOLOCK) --Change To Source Table
		WHERE 1=1
	--	AND DATEDIFF(DAY,z.[Last Modified Date],GETDATE())<=10 OR DATEDIFF(DAY,z.[Opt In Date],GETDATE())<=10 --This needs to be uncommented out and updated to the correct fields after the first load, update the date fields
	) a
