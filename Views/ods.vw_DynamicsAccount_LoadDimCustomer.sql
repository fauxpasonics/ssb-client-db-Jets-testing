...
CREATE VIEW [ods].[vw_DynamicsAccount_LoadDimCustomer] AS ( --Change "Source" to the correct source
 
    SELECT *
    /*Name*/
    , HASHBYTES('sha2_256',
...
 
            /*AddressPrimary*/
            , ISNULL(address1_line1,'') + ' ' + ISNULL(address1_line2,'') AS [AddressPrimaryStreet] -- dont load suites combine addresses with multiple lines and load to street
...
            , CASE WHEN (SELECT value FROM [dbo].[fn_Split](str_number,'| ') WHERE position = 1) IS NOT NULL
				THEN (SELECT value FROM [dbo].[fn_Split](str_number,'| ') WHERE position = 1) ELSE NULL END AS  [AccountId]
			, statecode AS IsDeleted
            , 1 AS IsBusiness
            , CASE WHEN (SELECT value FROM [dbo].[fn_Split](str_number,'| ') WHERE position = 1) IS NOT NULL
				THEN 'TM-' + (SELECT value FROM [dbo].[fn_Split](str_number,'| ') WHERE position = 1) ELSE NULL
				END customer_matchkey /*Standard for macthkeys are TM-xxxx, Pac-xxxx depending on source, 
                                            Customer Matchkey needs to have fairly complicated logic. 
                                            See Writup for details.*/

--		select top 100 *           
        FROM Jets_Reporting.prodcopy.account z WITH (NOLOCK) 
        WHERE 1=1
        AND (z.createdon > DATEADD(DAY,-3,GETDATE()) OR z.modifiedon > DATEADD(DAY,-3,GETDATE()))
