...
CREATE VIEW [etl].[vw_Load_DimCustomer_IMC_NYJCRM2011] AS 
...
, HASHBYTES('sha2_256', ISNULL(RTRIM(ExtAttribute31),'DBNULL_TEXT') 
	...
	--base set
		SELECT
			...
			, 'Silverpop_nyj2011' AS [SourceSystem] --Put source name such as TM or CRM_Contact
			, NULL AS [SourceSystemPriority] --Depends on if client uses sourcesystem/sourcesystempriority tables for MDM Rules

			/*Standard Attributes*/
			, CAST(cust.RECIPIENT_ID AS NVARCHAR(50)) [SSID] -- Required
			...
			, CAST(cust.Date_of_Birth AS DATETIME) AS [Birthday] -- field is sensitive see writup in sharepoint for an example of how complicated it can get
			, NULL AS [Gender] 
			, 0 [MergedRecordFlag]
			, NULL [MergedIntoSSID]
			...
			/*AddressPrimary*/
			, cust.STH_Street_1 AS [AddressPrimaryStreet]
			, cust.STH_City AS [AddressPrimaryCity] 
			, cust.STH_State AS [AddressPrimaryState] 
			, cust.STH_Zip AS [AddressPrimaryZip] 
			...
			/*Phone*/
			, LEFT(cust.STH_Phone_Number,25) AS [PhonePrimary]
			...
			, 'IMC-' + cust.RECIPIENT_ID AS[ExtAttribute6] --nvarchar(100)
			...
			, cust.Opt_In_Date AS SSUpdatedDate
			, ETL__CreatedDate	CreatedDate
			, GETDATE()	UpdatedDate
			, NULL AS IsDeleted
			, NULL [AccountId]
			, 0 AS IsBusiness
			, 'IMC-' + cust.RECIPIENT_ID AS [Customer_Matchkey] /*Standard for macthkeys are ‘TM-xxxx’, ‘Pac-xxxx’ depending on source, 
											Customer Matchkey needs to have fairly complicated logic. 
											See Writup for details.*/

--		select top 100 *			
		FROM stg.IMC_NotoriousBIG cust WITH (NOLOCK) 
		WHERE 1=1
		AND cust.ETL__CreatedDate > DATEADD(DAY,-3,GETDATE()) --This needs to be uncommented out after the first load, update the date fields
	) a
