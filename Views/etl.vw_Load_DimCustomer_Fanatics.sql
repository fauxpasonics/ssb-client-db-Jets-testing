...
CREATE VIEW [etl].[vw_Load_DimCustomer_Fanatics] AS 
...
, HASHBYTES('sha2_256', ISNULL(RTRIM(ExtAttribute31),'DBNULL_TEXT') 
	...
	--base set
		SELECT
			'Jets' AS [SourceDB] --Client Source Database such as Raiders or Raiders_Reporting
			, 'Fanatics' AS [SourceSystem] --Put source name such as TM or CRM_Contact
			, NULL AS [SourceSystemPriority] --Depends on if client uses sourcesystem/sourcesystempriority tables for MDM Rules

			/*Standard Attributes*/
			, CAST(cust.Client_ID AS NVARCHAR(50)) [SSID] -- Required
			...
			, CAST(NULL AS DATETIME) AS [Birthday] -- field is sensitive see writup in sharepoint for an example of how complicated it can get
			, NULL AS [Gender] 
			, 0 [MergedRecordFlag]
			, NULL [MergedIntoSSID]
			...
			/*Name*/
			, NULL AS [Prefix]
			, cust.Client_First_Name AS [FirstName]
			, NULL AS [MiddleName]
			, cust.Client_Last_Name AS [LastName]
			, NULL AS [Suffix]
			, NULL AS [Fullname]
			--, c.name_title as [Title]

			/*AddressPrimary*/
			, CASE WHEN ISNULL(cust.BillAddress2,'')<>'' THEN cust.BillAddress1+' '+cust.BillAddress2 WHEN ISNULL(cust.BillAddress2,'')='' THEN cust.BillAddress1 END AS [AddressPrimaryStreet]
			, cust.BillAddressCity AS [AddressPrimaryCity] 
			, cust.BillAddressState AS [AddressPrimaryState] 
			, cust.BillAddressZip AS [AddressPrimaryZip] 
			, NULL AS [AddressPrimaryCounty] 
			, cust.BilAddressCountry AS [AddressPrimaryCountry] 

            , CASE WHEN ISNULL(cust.ShipAddress2,'')<>'' THEN cust.ShipAddress1+' '+cust.ShipAddress2 WHEN ISNULL(cust.ShipAddress2,'')='' THEN cust.ShipAddress1 END AS [AddressOneStreet] -- dont load suites combine addresses with multiple lines and load to street
			, cust.ShipAddressCity AS [AddressOneCity] 
			, cust.ShipAddressState	AS [AddressOneState] 
			, cust.ShipAddressZip AS [AddressOneZip] 
			, cust.ShipAddressCounty AS [AddressOneCounty]
			...
			/*Phone*/
			, LEFT(cust.ShipAddressTel,25) AS [PhonePrimary]
			...
			/*Email*/
			, CAST(cust.Client_Email AS NVARCHAR(256)) AS [EmailPrimary]
			...
			, 'FAN-' + cust.Client_ID AS[ExtAttribute6] --nvarchar(100)
			...
			, SSCreatedDate  
			, SSUpdatedDate
			, ETL_CreatedDate	CreatedDate
			, ETL_UpdatedDate	UpdatedDate
			, cust.ETL_IsDeleted AS IsDeleted
			, NULL [AccountId]
			, 0 AS IsBusiness
			, 'FAN-' + cust.Client_ID AS [Customer_Matchkey] /*Standard for macthkeys are ‘TM-xxxx’, ‘Pac-xxxx’ depending on source, 
											Customer Matchkey needs to have fairly complicated logic. 
											See Writup for details.*/

--		select top 100 *			
		FROM ods.vw_Fanatics_Customers cust WITH (NOLOCK) 
		WHERE 1=1
