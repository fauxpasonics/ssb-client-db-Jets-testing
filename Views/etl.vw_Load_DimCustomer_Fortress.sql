...
CREATE VIEW [etl].[vw_Load_DimCustomer_Fortress] AS 
...
, HASHBYTES('sha2_256', ISNULL(RTRIM(ExtAttribute31),'DBNULL_TEXT') 
	...
	--base set
		...
		DISTINCT
			...
			, 'Fortress' AS [SourceSystem] --Put source name such as TM or CRM_Contact
			, NULL AS [SourceSystemPriority] --Depends on if client uses sourcesystem/sourcesystempriority tables for MDM Rules

			/*Standard Attributes*/
			, CAST(cust.patronID AS NVARCHAR(50)) [SSID] -- Required
			...
			, cust.addressName AS [SalutationName]
			...
			, NULL AS [LastName]
			, NULL AS [Suffix]
			, cust.addressName AS [Fullname]
			--, c.name_title as [Title]

			/*AddressPrimary*/
			, cust.AddressLine1 AS [AddressPrimaryStreet]
			, cust.AddressLine2 AS [AddressPrimaryCity] 
			, cust.AddressLine3 AS [AddressPrimaryState] 
			, cust.postCode AS [AddressPrimaryZip] 
			...
			/*Phone*/
			, LEFT(cust.phoneNumber,25) AS [PhonePrimary]
			...
			, 'Fort-' + cust.patronID AS[ExtAttribute6] --nvarchar(100)
			...
			, NULL SSCreatedDate  
			, NULL SSUpdatedDate
			, ETL_CreatedOn	CreatedDate
			, ETL_UpdatedOn	UpdatedDate
			, 0 AS IsDeleted
			, NULL [AccountId]
			, 0 AS IsBusiness
			, 'Fort-' + cust.patronID AS [Customer_Matchkey] /*Standard for macthkeys are ‘TM-xxxx’, ‘Pac-xxxx’ depending on source, 
											Customer Matchkey needs to have fairly complicated logic. 
											See Writup for details.*/

--		select top 100 *			
		FROM [ods].[Fortress_MemberInformation] cust WITH (NOLOCK) 
		WHERE 1=1
