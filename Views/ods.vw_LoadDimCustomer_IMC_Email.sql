...
CREATE VIEW [ods].[vw_LoadDimCustomer_IMC_Email] AS ( --Change "SOURCESYSTEM" to the correct source
...
--base set
		...
		DISTINCT
			...
			, 'IBM_Email' AS [SourceSystem] --Put source name such as TM or CRM_Contact
			,NULL AS [SourceSystemPriority] --Depends on if client uses sourcesystem/sourcesystempriority tables for MDM Rules

			/*Standard Attributes*/
			, CAST(z.RECIPIENT_ID AS NVARCHAR(200)) [SSID] -- Required, must be cast as NVARCHAR(200)
			...
			, CONVERT(VARCHAR(30),z.birthday, 102) AS [Birthday] -- field is sensitive see writup in sharepoint for an example of how complicated it can get
			, NULL AS [Gender] 
			, 0 [MergedRecordFlag]
			, NULL [MergedIntoSSID]
			...
			/*AddressPrimary*/
            , CASE WHEN ISNULL(z.Address_Street,'') != '' AND ISNULL(z.Address_Street_2,'') != '' THEN 
					z.Address_Street + ' ' + z.Address_Street_2
					ELSE z.Address_Street
					END AS [AddressPrimaryStreet] -- dont load suites combine addresses with multiple lines and load to street eg. CASE WHEN ISNULL(address2,'') <> '' THEN z.street + ' ' + z.address2 ELSE z.street END AS [AddressPrimaryStreet]
			, NULL AS [AddressPrimarySuite] -- ALWAYS LEAVE THIS NULL!!!!!!!!!!!!!!!!
			, z.Address_City AS [AddressPrimaryCity] 
			, z.Address_State AS [AddressPrimaryState] 
			, z.Address_Zip_Code AS [AddressPrimaryZip] 
			...
			/*Phone*/
			, LEFT(z.Primary_Phone_Number,25) AS [PhonePrimary] -- For Coalescing values make sute they are wrapped in a NULLIF for values that = '' eg COALESCE(NULLIF(z.mobile,''),z.phone) AS [PhonePrimary]
			...
			/*Email*/
			, CAST(z.Email AS NVARCHAR(256)) AS [EmailPrimary]
			...
			, z.[Opt_In_Date] [SSCreatedDate] --createddate
			, z.[ETL__UpdatedDate] [SSUpdatedDate] --lastmodifiedbyid
			...
			, 'CRM' + CAST(z.CRM_GUID AS NVARCHAR(50)) AS [Customer_Matchkey] /*Standard for macthkeys are ‘TM-xxxx’, ‘Pac-xxxx’ depending on source, 
		...
		--SELECT COUNT(*)			
		FROM [ods].IMC_Master z WITH (NOLOCK) --Change To Source Table
		WHERE 1=1
	--	AND DATEDIFF(DAY,z.[Opt_In_Date],GETDATE())<=10 --This needs to be uncommented out and updated to the correct fields after the first load, update the date fields
	) a
